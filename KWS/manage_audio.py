...
class AudioPreprocessor(object):
    def __init__(self, sr=16000, n_dct_filters=40, n_mels=40, f_max=8000, f_min=20, n_fft=480, hop_ms=10, alpha=[], tf=[], sigma_level = 0):
        super().__init__()
        self.n_mels = n_mels
        # self.dct_filters = librosa.filters.dct(n_dct_filters, n_mels)
        self.dct_filters = fftpack.dct(np.eye(n_mels), type=2, norm='ortho', n=n_dct_filters)
        self.sr = sr
    ...
    def compute_mfccs_full(self, data):
        data = np.abs(librosa.stft(data,
                            hop_length=self.hop_length,
                            n_fft=self.n_fft))
        data = data**2
...
class AudioSnippet(object):
    ...
    _dct_filters = fftpack.dct(np.eye(40), type=2, norm='ortho')
